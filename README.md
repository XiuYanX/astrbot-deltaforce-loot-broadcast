...
- 通知路由改为优先遵循 AstrBot 官方会话规则，基于 `unified_msg_origin`、`MessageType` 和 `event.is_private_chat()` 识别私聊/群聊
- 绑定失效、凭证异常等敏感提醒继续只发用户私聊；连续上游异常、物品目录自动刷新失败等系统告警会优先发给 AstrBot `admins_id` 管理员私聊
- 物品目录缓存改为每 12 小时自动过期刷新，刷新失败时会沿用旧缓存继续检测并主动发出系统告警
- 长时间重复的非鉴权异常会主动提醒一次，但不会误判成绑定失效；明确鉴权失效则会进入“已失效绑定”状态
- 补充官方路由回归测试，覆盖管理员私聊告警、私聊目标推导和失效提醒路由
