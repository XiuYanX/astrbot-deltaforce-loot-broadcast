...
- QQ 换取令牌阶段改为优先复用扫码前官方 `authorize/show` 授权页的原始上下文，并补充 `Origin` 与 `update_auth` 对齐，避免授权请求再次被 QQ 打回展示页
- 取码阶段会保存官方授权页 URL 与 `Q.isNeedLogin`，换令牌时继续沿用这一上下文，更贴近官网真实浏览器流程
- QQ 换取令牌阶段改为先回访官方 `authorize/show` 页面，再按官网脚本字段提交授权，兼容 JSON `callback` 形式的成功回调
